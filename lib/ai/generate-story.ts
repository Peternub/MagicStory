...
function buildPrompt(child: ChildProfile, request: StoryInput) {
  return [
    ...
    `Главная цель истории: ${request.goal}.`,
...
function generateFallbackStory({
  ...
  const text = [
    `${child.name} ${words.sat} на краю кровати и смотрел${child.gender === "girl" ? "а" : ""} на носок, который почему-то никак не хотел надеваться ровно. Пятка торчала сбоку, как маленький бугорок, и это уже немного смешило. День был непростой: ${request.situation}.`,
    ...
    `Первым уголком стало простое действие. ${child.name} ${words.fixed} носок. Потом сделал${child.gender === "girl" ? "а" : ""} маленький вдох. Потом сказал${child.gender === "girl" ? "а" : ""} одну короткую фразу про то, чего хочется: ${request.goal}. Фраза вышла не идеальная, зато своя.`,
    ...
    request.extraWishes
      ? `Потом ${child.name} вспомнил еще одну важную штуку: ${request.extraWishes}. Это не решило весь день за секунду, но помогло выбрать, что делать дальше.`
