...
export function buildSeriesPrompt(child: ChildProfile, request: StoryInput) {
  return [
    ...
    `Главный герой каждой серии — ${child.name}. Ребенок должен сразу узнавать в герое себя, а не просто читать историю о посторонних персонажах.`,
    "",
    "КАНОН И ФАКТЫ:",
    ...
    "- поведение, речь и решения главного героя должны соответствовать указанному возрасту;",
    ...
    "2. Есть ли минимум две узнаваемые детали, которые родитель действительно указал?",
